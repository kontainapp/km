...
To get started, read the introduction and technical user whitepaper, then build and test and from there
...
* The majority of formatting is handled by Visual Studio Code.
  * We use have clang-format forcing to be compliant with tabs and the likes.
...
* try `make help` to get more info
* `make test MATCH=regexp` will build and only run tests matching regexp
* tests/*.bats is were the test control is
