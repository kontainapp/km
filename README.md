...
Kontain is compatible with existing Docker and Kubernetes containers, and can be used to 
run your existing Container workloads in a manner that is "secure, fast and small—choose three" 
and with no changes to CI/CD tooling. 
Kontain works as a runtime for Docker and Kubernetes to run existing Containers 
with virtual machine level isolation and security,  but without traditional VM overhead. 
In fact, Kontain can generate smaller artifacts with much faster startup times 
than "classic" Docker containers. 
