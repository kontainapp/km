...
The logging facilities are controlled with 2 krun command line arguments
```
...
Since krun is normally run from docker or podman you need to arrange for the crun logging options to be supplied from these tools.

For  podman  there  is  an undocumented command line flag:
```
--runtime-flag.
```
...
```
podman run --runtime-flag log-format=json --runtime-flag log=file:/a/b/c .......
```
The omission of the leading ---- above is required.

For docker, you must supply the krun debug arguments with an entry in /etc/docker/daemon.json.
```
[root@localhost home]# cat /etc/docker/daemon.json
{  “runtimes”:
  {  “krun”:
    {
       “path”:  “/opt/kontain/bin/krun”,  “runtimeArgs”:  [  “--log=/home/krun.log”,  “--log-format=json”  ]
    }
  }
}
```
Maybe the docker documentation for daemon.json  will be of some help:
```
https://docs.docker.com/engine/reference/commandline/dockerd/#docker-runtime-execution-options
```
That page also says this: “Defining runtime arguments via the command line is not supported.”
So, it is quite cumbersome to turn on runtime debug with docker.

crun will in some places set the logging target back to stderr so if you think a trace might be missing it may be on stderr.
...
There may be several copies of crun (via fork()) with the same top level parent process, but the logging functions do not log the
...
# Configure podman and docker to use krun as a runtime
...
The script docker_config.sh in this directory is provided to configure docker to use krun as a runtime.
...
If it fails, you need to find and fix the problem.  It should be noted that this workflow does check coding style.
We need to follow the containers/crun coding style.
...
Another markdown file in this directoru discusses this:
keeping-krun-fork-current.md
