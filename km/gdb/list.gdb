...
if $argc == 0
   ...
   while $item != 0
      ...
      printf "%3d 0x%-12lx next 0x%-12lx start= 0x%lx size = %ld MiB (%ld) prot 0x%x flags 0x%x km_fl 0x%x distance %d MiB (%ld)\n", \
               $count,$item, $item->link->tqe_next, $item->start, $item->size/(1024ul*1024ul), \
               $item->size, $item->protection, $item->flags, $item->km_flags.data32, $distance/ (1024ul*1024ul), $distance
      set $count++
