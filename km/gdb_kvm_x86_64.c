...
static int kvm_arch_insert_sw_breakpoint(km_vcpu_t *vcpu, struct breakpoint_t *bp)
{
    ...
    uint8_t *insn = (uint8_t *)km_gva_to_kma(bp->addr); // TBD: we do need to check the addr+ size is still within the mem
    if ( insn == NULL ) {
 ...
 */
static struct breakpoint_t *bp_list_insert(gdb_breakpoint_type_t type,
                                           km_gva_t addr,
                                           size_t len)
{
    struct breakpoint_t *bp;
 ...
 */
static int bp_list_remove(gdb_breakpoint_type_t type,
 ...
 */
int km_gdb_read_registers(km_vcpu_t *vcpu,
                            uint8_t *registers,
                            size_t *len)
{
    ...
    ret = ioctl(vcpu->kvm_vcpu_fd, KVM_GET_REGS, &kregs);
    if (ret == -1) {
        err(1, "KVM_GET_REGS");
        return -1;
    }

    ret = ioctl(vcpu->kvm_vcpu_fd, KVM_GET_SREGS, &sregs);
    if (ret == -1) {
        err(1, "KVM_GET_REGS");
        return -1;
    }
    ...
    *len = sizeof(struct km_gdb_regs);
    ...
    // TBD: Add KVM_GET_FPU
    ret = ioctl(vcpu->kvm_vcpu_fd, KVM_GET_FPU, &fpuregs);
    if (ret == -1) {
 ...
 * Returns 0 if success, -1 otherwise.
 */
int km_gdb_write_registers(km_vcpu_t *vcpu,
                             uint8_t *registers,
                             size_t len)
{
    struct kvm_regs kregs;
 ...
 * Returns 0 if success, -1 otherwise.
 */
int km_gdb_add_breakpoint(km_vcpu_t *vcpu,
                            gdb_breakpoint_type_t type,
                            km_gva_t addr, size_t len)
{
    struct breakpoint_t *bp;
 ...
 * Returns 0 if success, -1 otherwise.
 */
int km_gdb_remove_breakpoint(km_vcpu_t *vcpu, gdb_breakpoint_type_t type,
                             km_gva_t addr, size_t len)
{
   struct breakpoint_t *bp;
