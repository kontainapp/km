...
static char* km_resolve_cmdname(const char* cmdname, const char* path, char* temp)
{
   // try path if specified, and the file name isn't full
   if (path != NULL && *cmdname != '/' && strncmp(cmdname, "./", 2) != 0 &&
       strncmp(cmdname, "../", 3 != 0)) {
      km_infox(KM_TRACE_EXEC, "Using PATH=%s", path);
      // Parse the value of the PATH var and try to find the path of the payload
      char* trailingcolon;
      for (const char* pvp = path; pvp != NULL; pvp = trailingcolon) {
         ...
         if (trailingcolon == NULL) {   // no trailing colon
            ...
            strncpy(temp, pvp, trailingcolon - pvp);
         }
         strcat(temp, "/");
         strcat(temp, cmdname);
         struct stat statbuf;
...
char** km_exec_build_argv(char* filename, char** argv, char** envp)
{
   ...
   if (strcmp(filename, SHELL_PATH) == 0) {   // so that popen() calls from the payload can work
      assert(argv[0] != NULL && argv[1] != NULL && argv[2] != NULL && argv[3] == NULL);
      ...
      filename = km_resolve_cmdname(nargv[0], km_payload_getenv("PATH", envp), buf);
   } else {
      shell = 0;
