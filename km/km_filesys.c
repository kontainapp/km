...
static inline km_filesys_t* km_filesys()
{
...
{
   assert(host_fd >= 0 && host_fd < km_filesys()->nfdmap);
   assert(start_guestfd >= 0 && start_guestfd < km_filesys()->nfdmap);
   int guest_fd = -1;
   for (int i = start_guestfd; i < km_filesys()->nfdmap; i++) {
      int fd_available = -1;
      if (__atomic_compare_exchange_n(&km_filesys()->guestfd_to_hostfd_map[i],
                                      &fd_available,
                                      host_fd,
                                      0,
                                      __ATOMIC_SEQ_CST,
                                      __ATOMIC_SEQ_CST) != 0) {
         __atomic_store_n(&km_filesys()->hostfd_to_guestfd_map[host_fd], i, __ATOMIC_SEQ_CST);
         void* newval = NULL;
         if (name != NULL) {
            newval = strdup(name);
            assert(newval != NULL);
         ...
         void* oldval = NULL;
         __atomic_exchange(&km_filesys()->guestfd_to_name_map[i], &newval, &oldval, __ATOMIC_SEQ_CST);
         if (oldval != NULL) {
            free(oldval);
         }
         guest_fd = i;
...
{
   assert(hostfd >= 0 && hostfd < km_filesys()->nfdmap);
   int rc = __atomic_compare_exchange_n(&km_filesys()->hostfd_to_guestfd_map[hostfd],
                                        &guestfd,
                                        -1,
                                        0,
                                        __ATOMIC_SEQ_CST,
   ...
   assert(guestfd >= 0 && guestfd < km_filesys()->nfdmap);
   rc = __atomic_compare_exchange_n(&km_filesys()->guestfd_to_hostfd_map[guestfd],
                                    &hostfd,
                                    -1,
                                    0,
                                    __ATOMIC_SEQ_CST,
   ...
   void* newval = NULL;
   void* oldval = NULL;
   __atomic_exchange(&km_filesys()->guestfd_to_name_map[guestfd], &newval, &oldval, __ATOMIC_SEQ_CST);
   if (oldval != NULL) {
      free(oldval);
   }
}
...
{
   if (fd < 0 || fd >= km_filesys()->nfdmap) {
      return NULL;
   }
   return km_filesys()->guestfd_to_name_map[fd];
}
...
{
   assert(guest_fd >= 0 && guest_fd < km_filesys()->nfdmap);
   assert(host_fd >= 0 && host_fd < km_filesys()->nfdmap);
   int close_fd =
       __atomic_exchange_n(&km_filesys()->guestfd_to_hostfd_map[guest_fd], host_fd, __ATOMIC_SEQ_CST);
   __atomic_store_n(&km_filesys()->hostfd_to_guestfd_map[host_fd], guest_fd, __ATOMIC_SEQ_CST);
...
{
   if (hostfd < 0 || hostfd >= km_filesys()->nfdmap) {
      return -ENOENT;
   }
   int guest_fd = __atomic_load_n(&km_filesys()->hostfd_to_guestfd_map[hostfd], __ATOMIC_SEQ_CST);
   if (__atomic_load_n(&km_filesys()->guestfd_to_hostfd_map[guest_fd], __ATOMIC_SEQ_CST) != hostfd) {
      guest_fd = -ENOENT;
   }
   return guest_fd;
}
 ...
 */
int km_guestfd_to_hostfd(int fd)
{
   if (fd < 0 || fd >= km_filesys()->nfdmap) {
      return -1;
   }
   int ret = __atomic_load_n(&km_filesys()->guestfd_to_hostfd_map[fd], __ATOMIC_SEQ_CST);
   assert((ret == -1) || (km_filesys()->hostfd_to_guestfd_map[ret] == fd) ||
          (km_filesys()->hostfd_to_guestfd_map[ret] == -1));
   return ret;
}

int km_fs_max_guestfd()
{
   return km_filesys()->nfdmap;
}
...
int km_fs_init(void)
{
   ...
   km_filesys()->guestfd_to_hostfd_map = malloc(mapsz);
   assert(km_filesys()->guestfd_to_hostfd_map != NULL);
   memset(km_filesys()->guestfd_to_hostfd_map, 0xff, mapsz);
   km_filesys()->hostfd_to_guestfd_map = malloc(mapsz);
   assert(km_filesys()->hostfd_to_guestfd_map != NULL);
   memset(km_filesys()->hostfd_to_guestfd_map, 0xff, mapsz);
   km_filesys()->nfdmap = lim.rlim_cur;
   km_filesys()->guestfd_to_name_map = calloc(lim.rlim_cur, sizeof(char*));
   assert(km_filesys()->guestfd_to_name_map != NULL);

   if (km_exec_recover_guestfd() != 0) {
      // setup guest std file streams.
      for (int i = 0; i < 3; i++) {
         km_filesys()->guestfd_to_hostfd_map[i] = i;
         km_filesys()->hostfd_to_guestfd_map[i] = i;
         switch (i) {
            case 0:
               km_filesys()->guestfd_to_name_map[i] = strdup("[stdin]");
               break;
            case 1:
               km_filesys()->guestfd_to_name_map[i] = strdup("[stdout]");
               break;
            case 2:
               km_filesys()->guestfd_to_name_map[i] = strdup("[stderr]");
               break;
         }
         assert(km_filesys()->guestfd_to_name_map[i] != NULL);
      }
   }
   return 0;
}
...
{
   for (int i = 0; i < 3; i++) {
      free(km_filesys()->guestfd_to_name_map[i]);
   }
   if (km_filesys()->guestfd_to_name_map != NULL) {
      free(km_filesys()->guestfd_to_name_map);
      km_filesys()->guestfd_to_name_map = NULL;
   }
   if (km_filesys()->guestfd_to_hostfd_map != NULL) {
      free(km_filesys()->guestfd_to_hostfd_map);
      km_filesys()->guestfd_to_hostfd_map = NULL;
   }
   if (km_filesys()->hostfd_to_guestfd_map != NULL) {
      free(km_filesys()->hostfd_to_guestfd_map);
      km_filesys()->hostfd_to_guestfd_map = NULL;
   }
}
...
uint64_t km_fs_openat(km_vcpu_t* vcpu, int dirfd, char* pathname, int flags, mode_t mode)
{
   ...
   if (dirfd != AT_FDCWD && pathname[0] != '/') {
      if ((host_dirfd = km_guestfd_to_hostfd(dirfd)) < 0) {
         return -EBADF;
      }
   }
   int guestfd;
...
uint64_t km_fs_close(km_vcpu_t* vcpu, int fd)
{
   ...
   if ((host_fd = km_guestfd_to_hostfd(fd)) < 0) {
      return -EBADF;
   }
   /*
...
{
   int host_fd;
   if ((host_fd = km_guestfd_to_hostfd(sockfd)) < 0) {
      return -EBADF;
   }
...
static int readlink_procself(const char* pathname, char* buf, size_t bufsz)
{
   ...
   char* mpath;
   if (fd < 0 || fd >= km_filesys()->nfdmap || (mpath = km_filesys()->guestfd_to_name_map[fd]) == 0) {
      return -ENOENT;
   }
   /*
...
km_fs_statx(km_vcpu_t* vcpu, int dirfd, char* pathname, int flags, unsigned int mask, void* statxbuf)
{
   ...
   if (dirfd != AT_FDCWD && pathname[0] != '/') {
      if ((host_fd = km_guestfd_to_hostfd(dirfd)) < 0) {
         return -EBADF;
      }
   }
...
uint64_t km_fs_dup3(km_vcpu_t* vcpu, int fd, int newfd, int flags)
{
   ...
   if (newfd < 0 || newfd >= km_filesys()->nfdmap) {
      return -EBADF;
   }
   ...
   int host_newfd = km_guestfd_to_hostfd(newfd);
...
{
   int host_sockfd;
   if ((host_sockfd = km_guestfd_to_hostfd(sockfd)) < 0) {
      return -EBADF;
   }
   int ret =
...
uint64_t km_fs_sendfile(km_vcpu_t* vcpu, int out_fd, int in_fd, off_t* offset, size_t count)
{
   ...
   if ((host_outfd = km_guestfd_to_hostfd(out_fd)) < 0) {
      return -EBADF;
   }
   if ((host_infd = km_guestfd_to_hostfd(in_fd)) < 0) {
      return -EBADF;
   }
...
{
   int host_outfd, host_infd;
   if ((host_outfd = km_guestfd_to_hostfd(fd_out)) < 0) {
      return -EBADF;
   }
   if ((host_infd = km_guestfd_to_hostfd(fd_in)) < 0) {
      return -EBADF;
   }
                      ...
                      struct timeval* timeout)
{
   ...
   if (ret > 0) {
      for (int i = 0; i < nfds; i++) {
         int guest_fd = km_hostfd_to_guestfd(i);
         if (guest_fd < 0) {
            continue;
         }
...
uint64_t km_fs_poll(km_vcpu_t* vcpu, struct pollfd* fds, nfds_t nfds, int timeout)
{
   ...
   for (int i = 0; i < nfds; i++) {
      if ((host_fds[i].fd = km_guestfd_to_hostfd(fds[i].fd)) < 0) {
         return -EBADF;
      }
...
uint64_t km_fs_epoll_ctl(km_vcpu_t* vcpu, int epfd, int op, int fd, struct epoll_event* event)
{
   ...
   if ((host_epfd = km_guestfd_to_hostfd(epfd)) < 0 || (host_fd = km_guestfd_to_hostfd(fd)) < 0) {
      return -EBADF;
   }
...
{
   int host_epfd;
   if ((host_epfd = km_guestfd_to_hostfd(epfd)) < 0) {
      return -EBADF;
   }
                         ...
                         struct rlimit* old_limit)
{
    ...
    */
   if (resource == RLIMIT_NOFILE && new_limit != NULL && new_limit->rlim_cur > km_filesys()->nfdmap) {
      return -EPERM;
   }
