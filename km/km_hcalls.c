...
static km_hc_ret_t sigaltstack_hcall(void* vcpu, int hc, km_hc_args_t* arg)
{
   ...
   if ((new = km_gva_to_kma(arg->arg1)) == NULL && arg->arg1 != 0) {
      arg->hc_ret = -EFAULT;
      return HC_CONTINUE;
   }
   if ((old = km_gva_to_kma(arg->arg2)) == NULL && arg->arg2 != 0) {
      arg->hc_ret = -EFAULT;
      return HC_CONTINUE;
   }
   arg->hc_ret = km_sigaltstack(vcpu, new, old);
   km_infox(KM_TRACE_HC, "hc = %d (sigaltstack %s), %ld %lx", hc, km_hc_name_get(hc), arg->arg1, arg->arg2);
   return HC_CONTINUE;
}
