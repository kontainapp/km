...
static km_hc_ret_t uname_hcall(void* vcpu, int hc, km_hc_args_t* arg)
{
   ...
   if (arg->hc_ret == 0) {
      ...
      if (machine.vm_type == VM_TYPE_KKM) {
         strncat(name->release, ".kontain.KKM", sizeof(name->release) - 1);
      } else {
         strncat(name->release, ".kontain.KVM", sizeof(name->release) - 1);
      }
   }
...
void km_hcalls_init(void)
{
   ...
   km_hcalls_table[SYS_mknodat] = mknodat_hcall;
   km_hcalls_table[SYS_newfstatat] = newfstatat_hcall;
