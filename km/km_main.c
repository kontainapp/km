...
char* km_get_self_name()
{
   ...
   if (km_bin_name == NULL) {
      ...
      if ((bytes = readlink(PROC_SELF_EXE, buf, PATH_MAX)) < 0) {
         km_warn_msg("Failed to read %s", PROC_SELF_EXE);
         return NULL;
      }
      buf[bytes] = '\0';
...
char* km_parse_shebang(const char* payload_file, char** extra_arg)
{
   ...
   *extra_arg = NULL;
   ...
   if (count <= SHEBANG_LEN) {
      km_warn_msg("Failed to read even %ld bytes from %s", SHEBANG_LEN, payload_file);
      return NULL;
   }
...
km_parse_args(int argc, char* argv[], int* argc_p, char** argv_p[], int* envc_p, char** envp_p[])
{
   ...
   if ((pl_name = km_traverse_payload_symlinks((const char*)getauxval(AT_EXECFN))) != NULL ||
         ...
         switch (opt) {
            ...
            case 'g':   // enable the gdb server and specify a port to listen on
               if (optarg != NULL) {
                  ...
                  if (errno != 0 || (endp != NULL && *endp != 0)) {
                     km_warn_msgx("Invalid gdb port number '%s'", optarg);
                     usage();
                  }
               } else {
            ...
            case 'e':                    // --putenv
               if (copyenv_used > 1) {   // if --copyenv was on the command line, something is wrong
                  km_warn_msgx("Wrong options: '--putenv' cannot be used with together with "
                               "'--copyenv'");
                  usage();
               }
            ...
            case 'E':   // --copyenv
               if (putenv_used != 0) {
                  km_warn_msgx("Wrong options: '--copyenv' cannot be used with together with "
                               "'--putenv'");
                  usage();
               }
               copyenv_used++;
            ...
            case 'P':
               ...
               if (ep == NULL || *ep != '\0') {
                  km_warn_msgx("Wrong memory bus size '%s'", optarg);
                  usage();
               }
               if (gpbits < 32 || gpbits >= 63) {
                  km_warn_msgx("Guest memory bus width must be between 32 and 63 - got '%d'", gpbits);
                  usage();
               }
               km_machine_init_params.guest_physmem = 1UL << gpbits;
               if (km_machine_init_params.guest_physmem > GUEST_MAX_PHYSMEM_SUPPORTED) {
                  km_warn_msgx("Guest physical memory must be < 0x%lx - got 0x%lx (bus width %d)",
            ...
            case 'V':
               if (optarg == NULL) {
                  ...
                  km_warn_msgx("Failed to compile -V regexp '%s'", optarg);
                  usage();
               }
               km_info_trace.level = KM_TRACE_INFO;
               break;
      ...
      if (resume_snapshot != 0 && (putenv_used != 0 || copyenv_used != 0 || dynlinker_used != 0)) {
         km_err_msg(1, "cannot set new environment or dynlinker when resuming a snapshot");
      }
   }
...
int main(int argc, char* argv[])
{
   ...
   if (km_exec_recover_kmstate() < 0) {   // exec state is messed up
      km_err_msgx(2, "Problems in performing post exec processing");
   }

   if ((payload_name = km_parse_args(argc, argv, &argc_p, &argv_p, &envc, &envp)) == NULL) {
      km_warn_msgx("Failed to determine payload name or find .km file for %s", argv[0]);
      usage();
   }
   ...
   if (wait_for_signal == 1) {
      km_warn_msgx("Waiting for kill -SIGUSR1 %d", getpid());
   ...
   if (km_gdb_is_enabled() != 0) {
      if (km_gdb_setup_listen() == 0) {   // Try to become the gdb server
         ...
         km_warn_msgx("Failed to setup gdb listening port %d, disabling gdb support", gdbstub.port);
