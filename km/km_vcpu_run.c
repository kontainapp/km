 ...
 */
static uint64_t* km_reg_ptr(km_vcpu_t* vcpu, int b, int reg)
{
   if (b < 0 || b > 1) {
      return NULL;
   }
...
static void* km_faulting_address(km_vcpu_t* vcpu)
{
   km_read_registers(vcpu);

   /*
    * This hack looks for the 'TEST' instruction generated by OpenJDK JIT
    * for safepoint checking. It is the "memory64 and qwordregister" variant
    * of TEST. See SDM Volume 2, Appendix B for instruction encodings.
    * We only care about 64-Bit Mode.
    *
    * In particular, the intruction we care about is somethine like:
    *
    *    TEST    RAX, (R10)
    *
    * And is encoded:
    *   0100 XRXB 1000 0101: mod qwordreg r/m
    *
    * The first byte is a REX prefix (0x4X), the second byte is the opcode,
    * and the third byte is the ModR/M byte (mode:reg1:reg2).
    * We only care about reg2 of the ModR/M byte.
    */
   if (km_is_gva_accessable(vcpu->regs.rip, 3, PROT_READ | PROT_EXEC) == 0) {
   ...
   if ((instr[0] & 0xf0) == 0x40 && instr[1] == 0x85) {
      int b = instr[0] & 0x01;
      int mode = (instr[2] >> 6) & 0x3;
      // int reg1 = (instr[2] >> 3) & 0x7;
      int reg2 = instr[2] & 0x7;
      uint64_t* regp = km_reg_ptr(vcpu, b, reg2);
      if (mode == 0 && regp != NULL && km_is_gva_accessable(*regp, sizeof(uint64_t), PROT_READ) == 0) {
         km_infox(KM_TRACE_SIGNALS, "FAULT @0x%lx", *regp);
         return (void*)*regp;
      }
   }
   ...
   if (rc != 0) {
      ...
      switch (errno) {
         ...
         case EFAULT: {
            ...
            info.si_addr = km_faulting_address(vcpu);
            km_post_signal(vcpu, &info);
            break;
         }
         default:
