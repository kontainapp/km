...
void km_read_registers(km_vcpu_t* vcpu)
{
   ...
   // Use the copy of the registers KVM_RUN returns
   vcpu->regs = vcpu->cpu_run->s.regs.regs;
   vcpu->regs_valid = 1;
}
...
static int hypercall(km_vcpu_t* vcpu, int* hc)
{
   ...
   *hc = r->io.port - KM_HCALL_PORT_BASE;
   ...
   ga = r->s.regs.regs.rax;    // address of km_hc_args_t is by convention in rax
   km_infox(KM_TRACE_HC, "calling hc = %d (%s), arg gva 0x%lx", *hc, km_hc_name_get(*hc), ga);
   km_kma_t ga_kma;
   if ((ga_kma = km_gva_to_kma(ga)) == NULL || km_gva_to_kma(ga + sizeof(km_hc_args_t) - 1) == NULL) {
      km_infox(KM_TRACE_SIGNALS, "**** hc: %d km_hc_args_t address:0x%lx is invalid ****", *hc, ga);
...
static void km_vcpu_one_kvm_run(km_vcpu_t* vcpu)
{
   ...
   vcpu->sregs_valid = 0;
   vcpu->cpu_run->kvm_valid_regs = KVM_SYNC_X86_REGS;
