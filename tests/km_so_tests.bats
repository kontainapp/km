...
@test "km_main(shared): optargs (hello_test)" {
   ...
   assert_failure
   assert_line --partial "km: PT_INTERP does not contain km marker."
...
@test "gdb_server_race(shared): gdb server concurrent wakeup test" {
   skip "TODO: shared version needed"
#   km_gdb_default_port=2159
#   km_trace_file=/tmp/gdb_server_race_test_shared_$$.out
...
#   km_with_timeout -V -g gdb_server_entry_race_test.km >$km_trace_file 2>&1 &
#   gdb_pid=$! ; sleep 0.5
